                        ...
                        AVRational frame_rate, int pic_fmt)
{
    ...
    fprintf(stdout, "Init frame send interval: %d\r\n", interval.count());
    ...
    if (ret < 0)
    {
        fprintf(stderr, "avformat_alloc_output_context2 failed 0x%X\r\n", ret);
        return ret;
    }
    ...
    if (!(encoder_avfc->oformat->flags & AVFMT_NOFILE))
    {
        ...
        if (ret < 0)
        {
            ...
            fprintf(stderr, "avio_open2 failed err code: 0x%X Reason: %s\r\n", ret,
    ...
    if (ret < 0)
    {
        fprintf(stderr, "avcodec_parameters_from_context failed 0x%X", ret);
        return ret;
    }
    ...
    if (ret < 0)
    {
        ...
        fprintf(stderr, "avformat_new_stream failed code: 0x%X Reason: %s\r\n", ret,
                av_make_error_string(err_buf, AV_ERROR_MAX_STRING_SIZE, ret));
        return ret;
    }
...
void FFmpegEncoder::SendFrame(const uint8_t *pdata)
{
    ...
    if (ret < 0)
    {
        fprintf(stderr, "avcodec_send_frame failed 0x%X\r\n", ret);
    ...
    while (true)
    {
        ...
        if (ret < 0)
        {
            ...
            // fprintf(stderr, "avcodec_receive_packet failed 0x%X\r\n", ret);
        ...
        if (ret < 0)
        {
            ...
            fprintf(stderr, "av_interleaved_write_frame failed 0x%X\r\n", ret);
            return;
        }
    ...
    // std::cerr << "[FFmpegEncoder::SendFrame] End" << std::endl;
}
...
void FFmpegEncoder::Process(std::tuple<void *, uint32_t> buffer)
{
