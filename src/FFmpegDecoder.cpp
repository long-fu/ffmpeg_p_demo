...
{
    fprintf(stdout,"Set parameters for %s", streamName_.c_str());
    ...
    fprintf(stdout,"Set parameters for %s end", streamName_.c_str());
}
...
bool FFmpegDecoder::OpenVideo(AVFormatContext*& avFormatContext)
{
    ...
    fprintf(stdout,"Open video %s ...", streamName_.c_str());
    ...
    if (openRet < 0) { // check open video result
        ...
        fprintf(stderr,"Could not open video:%s, return :%d, error info:%s",
                                    ...
                                    AVBSFContext*& bsfCtx)
{
    ...
    if (videoFilter == nullptr) { // check video fileter is nullptr
        fprintf(stderr,"Unkonw bitstream filter, videoFilter is nullptr!");
        return false;
    }

    // checke alloc bsf context result
    if (av_bsf_alloc(videoFilter, &bsfCtx) < 0) {
        fprintf(stderr,"Fail to call av_bsf_alloc!");
        return false;
    }
    ...
    if (avcodec_parameters_copy(bsfCtx->par_in,
        ...
        fprintf(stderr,"Fail to call avcodec_parameters_copy!");
        return false;
    }
    ...
    if (av_bsf_init(bsfCtx) < 0) {
        fprintf(stderr,"Fail to call av_bsf_init!");
        return false;
    }
...
void FFmpegDecoder::Decode(FrameProcessCallBack callback,
                           void *callbackParam)
{
    fprintf(stdout,"Start ffmpeg decode video %s ...", streamName_.c_str());
    ...
    if (videoIndex == kInvalidVideoIndex) { // check video index is valid
        fprintf(stderr,"Rtsp %s index is -1", streamName_.c_str());
        return;
    }
    ...
    fprintf(stdout,"Start decode frame of video %s ...", streamName_.c_str());
        ...
        if (avPacket.stream_index == videoIndex) { // check current stream is video
          // send video packet to ffmpeg
            if (av_bsf_send_packet(bsfCtx, &avPacket)) {
                fprintf(stderr,"Fail to call av_bsf_send_packet, channel id:%s",
                    streamName_.c_str());
            }
    ...
    isFinished_ = true;
    fprintf(stdout,"Ffmpeg decoder %s finished", streamName_.c_str());
}
...
void FFmpegDecoder::GetVideoInfo()
{
    ...
    if (ret == false) {
        fprintf(stderr,"Open %s failed", streamName_.c_str());
        return;
    }

    if (avformat_find_stream_info(avFormatContext, NULL)<0) {
        fprintf(stderr,"Get stream info of %s failed", streamName_.c_str());
        return;
    }

    int videoIndex = GetVideoIndex(avFormatContext);
    if (videoIndex == kInvalidVideoIndex) { // check video index is valid
        fprintf(stderr,"Video index is %d, current media stream has no "
                          "video info:%s",
    ...
    fprintf(stdout,"Video %s, type %d, profile %d, width:%d, height:%d, fps:%d",
